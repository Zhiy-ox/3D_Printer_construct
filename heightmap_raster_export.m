...
PixelPitch    = [];
BaseHeight    = 0;       % source units; use 0.5 for a CSV that needs a 0.5 um base
...
try
    ...
    if Verbose
        ...
        fprintf(' Source pitch: %.10g x %.10g %s => %.10g x %.10g mm after scaling\n', ...
            hm.SourcePitch(1), hm.SourcePitch(2), hm.Units, ...
            info.SourcePitch_mm(1), info.SourcePitch_mm(2));
        fprintf(' Added base:   %.10g source units => %.10g mm\n', ...
            info.BaseHeightSource, info.BaseHeight_mm);
